...
exports.fetchFromCache = async (req, res, next) => {
  ...
  if (process.env.NODE_ENV === 'test' || req.method !== 'GET') {
    ...
    if (cached) {
      ...
      res.status(200).send(typeof cached === 'string' ? cached : JSON.parse(cached));
    } else {
      next();
    }
  }
 ...
 * @param {Number} ttl: number of seconds before cache expiration. Defaults to 60.
 */
exports.setCache = async (key, val, ttl = 60) => {
